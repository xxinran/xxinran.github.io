...
- 栈 `stack`
  ...
  >堆和栈中间空出来的部分可以做文件映射，把文件（的一部分）直接映射到内存上，避免read，write IO操作文件，更高效。要主要对齐页表。
...
一个页有两个部分组成，FrameIndex和位p（present存在位）。FrameIndex指向对应的物理页框，p指出本页的映射是否有效。还有另外一部分，访问控制信息，记录本页的访问权限，可读可写可执行等。 

有效：p=1, frameIndex存在。

无效：p=0，frameIndex不存在，没有被映射到物理内存。

如果发现p=0， CPU发生一个**缺页故障（page fault）**，系统必须从物理页框中找到1个当前很少被使用的页框并把该页框的内容写入外围存储器（这个动作被称为page copy），随后把本页映射到刚才释放的页框中（这个动作称为**修改映射关系**）

虚拟地址 = 页号索引（page Index）+ 页偏移量（Offset）

比如一个32位系统，每个进程有4GB虚拟地址，一共有4GB/4KB = 2^20个page，需要10bits（1024 = 2^20）来记录page index， 每个页里面有4KB（2^12），验证了虚拟地址是20+12=32位。

### TLB

在MMU工作的时候，需要有一张表来记录当前地址的映射关系，这张表存在内存中，每次地址转换都要去遍历这张表，效率很低，所以在MMU中增加了cache，这个cache就是TLB。



## 分页机制

页表保存在**物理内存中**，**MMU会查找页表来确定一个VA应该映射到什么PA。**太慢，所以有MMU的TLB。
